...
export default function EditIBANScreen({navigation, route}: Props) {
  const {updateIBAN, loading} = useIBAN();
  const {iban} = route.params;
  
  const [formData, setFormData] = useState({
    iban_number: formatIBANInput(iban.iban_number),
  ...
  const validateIBAN = async (ibanInput: string) => {
    const cleanedIBAN = cleanIBAN(ibanInput);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      Toast.show({
        type: 'success',
        text1: 'Başarılı',
        text2: 'IBAN başarıyla güncellendi',
      });
        ...
        >
          {loading ? (
            <View style={styles.buttonContent}>
              ...
              <Text style={styles.buttonText}>Güncelleniyor...</Text>
            </View>
          ) : (
            <View style={styles.buttonContent}>
